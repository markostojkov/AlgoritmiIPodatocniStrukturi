...
public class WindowsExplorer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        tree.makeRoot("c:");
        Node<String> currentRoot = tree.root();

        for (String command : commands) {
            String[] commandAndName = command.split(" ");

            switch(commandAndName[0]) {
                case "CREATE":
                    currentRoot = tree.addChild(currentRoot, commandAndName[1]);    
                    break;
                case "OPEN":
                    currentRoot = tree.root
                    break;
                case "DELETE":
                    tree.remove(new SLLNode<String>(commandAndName[1]));
                    break;
                case "BACK":
                    tree.makeRoot(tree.root().getElement());
                    break;
                case "PATH":
                    SLLNode<String> pathRoot = (SLLNode<String>) currentRoot;
                    String path = "";
                    while(pathRoot != null) {
                        path += pathRoot.getElement() + "\\";
